...
TOOLBOX_PATH = str(BASE_DIR / "ebook_toolbox")
...
def hunt_book(book_name):
    ...
    try:
        with open(CONFIG_FILE, 'r') as f:
            ...
            if remix_id and remix_key and remix_id != "Get_From_Browser_Cookie":
                zlib_client = Zlibrary(remix_userid=remix_id, remix_userkey=remix_key)
                print("汪！使用 Remix Token 登录...")
            elif email and password:
                zlib_client = Zlibrary(email=email, password=password)
                print("汪！使用账号密码登录...")
            else:
    ...
    query = "精益数据分析 阿利斯泰尔"
    try:
        results = zlib_client.search(message=query, languages="chinese")
        if results and results.get("books"):
            ...
            if best_book:  # Ensure best_book is not None before accessing
                ...
                author = best_book.get("author", "Unknown")
                print(f"目标锁定：{title} ({ext}) - 作者: {author}")
                
                # 3. Download
                print("正在试图叼回来... (这可能需要几分钟...)")
                filename, content = zlib_client.downloadBook(best_book)
            ...
            if content:
                ...
                if "epub" in ext.lower():
                    ...
                    import subprocess
                    converter_script = os.path.join(BASE_DIR, "epub_to_md.py")
                    result = subprocess.run(["py", converter_script, save_path], shell=True)
                    if result.returncode != 0:
                        print(f"呜... 消化程序退出异常: {result.returncode}")
